    ...
    where TRole : MongoDbRole<TKey>
{
    ...
    public async Task SetTokenAsync(TUser user, string loginProvider, string name, string value, CancellationToken cancellationToken)
    {
        ...
        if (token is null)
        {
            var userToken = new MongoDbUserToken(loginProvider, name, value);
    ...
    {
        return SetTokenAsync(user, _internalLoginProvider, _authenticatorKeyTokenName, key, cancellationToken);
    }
    ...
    public Task AddClaimsAsync(TUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken)
    {
        ...
        {
            var identityClaim = new MongoDbUserClaim(claim.Type, claim.Value);
    ...
    public Task AddLoginAsync(TUser user, UserLoginInfo login, CancellationToken cancellationToken)
    {
        ...
        var userLogin = new MongoDbUserLogin(login.LoginProvider, login.ProviderKey, login.ProviderDisplayName);
    ...
    private async Task AddToRoleInternalAsync(TUser user, string roleName, CancellationToken cancellationToken)
    {
        ...
        var userRole = new MongoDbUserRole<TKey>(roleEntity.Id, roleEntity.Name);
    ...
    private async Task<IList<TUser>> GetUsersInRoleInternalAsync(string roleName, CancellationToken cancellationToken)
    {
        ...
        var userRole = new MongoDbUserRole<TKey>(role.Id, role.Name);
    ...
    {
        return _roleCollection.FirstOrDefaultAsync(x => x.NormalizedName == normalizedRoleName.ToUpperInvariant(), cancellationToken);
    }

    private async Task<MongoDbUserToken?> FindTokenAsync(TUser user, string loginProvider, string name, CancellationToken cancellationToken = default)
    {
        var dbUser = await FindByIdAsync(user.Id, cancellationToken).ConfigureAwait(true);
    ...
    private Task<TUser> FindByIdAsync(TKey id, CancellationToken cancellationToken = default)
    {
